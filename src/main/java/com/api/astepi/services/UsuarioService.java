...
public class UsuarioService {
    @Autowired
    private UsuarioRepository usuarioRepository;    //Atualizado

    @Transactional
    public UsuarioModel addAgendamento(UUID usuarioId, AgendamentoDto agendamentoDto) {   //Atualizado AGENDAMENTO
    ...
    public UsuarioModel addEndereco(UUID usuarioId, EnderecoDto enderecoDto) {          //Atualizado ENDEREÇO
        Optional<UsuarioModel> usuarioOptional = usuarioRepository.findById(usuarioId);
        if (usuarioOptional.isEmpty()) {
            throw new ResourceNotFoundException("Usuário não encontrado");
        }
